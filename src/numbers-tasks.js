...
function roundToPowerOfTen(num, pow) {
  const i = 10 ** pow;
  return Math.round(Math.floor(num) / i) * i;
}
...
function toNumber(value, def) {
  ...
  let i;
  if (typeof a === 'number' && Number.isNaN(a) === !true) {
    i = a;
  } else {
    i = def;
  }
...
function getSumOfDigits(num) {
  ...
  const sum = numString.reduce((total, element) => +total + +element, 0);
  return sum;
}
...
function isPowerOfTwo(num) {
  return Number.isSafeInteger(Math.log2(num));
}
